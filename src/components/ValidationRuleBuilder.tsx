            ...
            <div className="flex items-center justify-between gap-2 mb-3">
                ...
                >
                    {expandedSection ? (
                        ...
                        <ChevronRight className="w-4 h-4 text-white/30 group-hover:text-white/50 transition-colors shrink-0" />
                    )}
                    <GitBranch className="w-4 h-4 text-violet-400/60 shrink-0" />
                    <h3 className="text-base sm:text-lg font-semibold text-white/70 truncate">Validation Rules</h3>
                    {hasContent && !expandedSection && (
                        <span className="ml-1 px-2 py-0.5 text-xs font-medium bg-violet-500/15 text-violet-300/70 rounded-md shrink-0">
                            {countConditions(rules.rootGroup)}
                        </span>
                    )}
                </button>
                <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 sm:gap-1.5 text-sm transition-colors"
                    ...
                    >
                        ...
                        <span className={`hidden sm:inline ${rules.clientEnabled ? 'text-violet-300/70' : 'text-white/20'}`}>
                            Client
                        </span>
                    </button>
                    ...
                    >
                        ...
                        <span className={`hidden sm:inline ${rules.serverEnabled ? 'text-emerald-300/70' : 'text-white/20'}`}>
                            Server
                        </span>
                    </button>
                </div>
...
function ConditionRow({ condition, fields, onUpdate, onDelete }: ConditionRowProps) {
    ...
    return (
        <div className={`flex flex-wrap sm:flex-nowrap items-center gap-2 group/row ${rowOpacity}`}>
            {/* Field select */}
            <select
